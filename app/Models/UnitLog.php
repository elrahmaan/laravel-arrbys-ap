...
class UnitLog extends Model
{
    ...
    public static function getDataUnit()
    {
        ...
        ->select('service_assets.name as asset_name', 'departments.name as department_name',
         'unit_logs.complainant_name as complainant_name','unit_logs.desc_complain as desc_complain', 'unit_logs.diagnose as diagnose',
    ...
    public static function getServiceParameter($fromDates, $toDates)
    {
        if ($fromDates == $toDates) {
            ...
            ->select('service_assets.name as asset_name', 'departments.name as department_name',
             'unit_logs.complainant_name as complainant_name','unit_logs.desc_complain as desc_complain', 'unit_logs.diagnose as diagnose',
            'unit_logs.date_fixed as date_fixed')->where('unit_logs.date_fixed', '=', $fromDates)->get()->toArray();
        } else {
        $record = DB::table('unit_logs')->leftJoin('service_assets','unit_logs.asset_id', '=' ,'service_assets.id')
        ->leftJoin('departments','unit_logs.department_id','=','departments.id')
        ->select('service_assets.name as asset_name', 'departments.name as department_name',
         'unit_logs.complainant_name as complainant_name','unit_logs.desc_complain as desc_complain', 'unit_logs.diagnose as diagnose',
        'unit_logs.date_fixed as date_fixed')->where('unit_logs.date_fixed', '>=', $fromDates)
